...
启动本地 Web 控制台，在浏览器中管理当前登录帐号的网盘文件：

```bash
BaiduPCS-Go web
```

默认访问地址为 `http://127.0.0.1:8080`，也可以通过 `--addr` 修改监听地址：

```bash
BaiduPCS-Go web --addr 127.0.0.1:9090
```

如果需要给 Web 页面增加访问密码，可以在启动时设置 `--password`：

```bash
BaiduPCS-Go web --password your-password
```

访问密码通过当前浏览器会话保存，服务重启后需要重新输入。通过访问密码后，仍需在页面内使用百度网盘 Cookie 登录。

Web 页面支持使用百度网盘 Cookie 登录或切换账号，并分为“文件概览”“文件上传”“文件下载”和“文件分享”四个标签页：概览页用于浏览目录、新建文件夹、重命名并直接接收文件，上传页支持浏览器文件上传和指定服务器本地文件/目录路径上传，并查看当前任务和上传历史，下载页可选择浏览器接收、加入服务器下载队列，或将服务器本地文件下载到当前浏览器，并查看下载进度和下载历史，分享页支持创建分享链接、设置有效期和提取码，并查看已有分享记录；文件列表按页展示。默认只监听本机地址；如果使用 `--addr 0.0.0.0:8080` 对外提供服务，建议同时设置 `--password` 并配合 HTTPS 或反向代理使用。

多人共用同一个百度账号时，每个浏览器会获得独立的 Web 会话，上传和下载中的任务只显示给发起任务的用户；上传和下载历史按共享账号统一显示。上传任务会串行执行，避免共用上传断点文件产生冲突，下载任务可以并行执行，并保存到 `WebDownloads/<session>` 独立目录，避免不同用户下载同名文件互相覆盖。
